...
var lineMarkersLayer = L.layerGroup();
var markers = new Map();
function load() {
    ...
    document.getElementById('lines_check')?.addEventListener('change', event => {
        if (event.currentTarget.checked) {
            lineMarkersLayer.show()
        } else {
            lineMarkersLayer.hide()
        }
    });
...
class Marker {
    constructor(response) {
        ...
        this._marker = L.circle(this._position, {
            ...
            fillOpacity: 1,
            radius: 50
        }).addTo(markersLayer).bindPopup(this.name);
        ...
        fetch(`/livemap/data/${response.detailsReference}`)
            .then(res => {
                ...
                this._lineMarker = L.polyline(data, {color: this.color}).addTo(lineMarkersLayer)
            });
    }
...
function drawPartitions() {
    ...
    for (let y = 0; y < partitions; y++) {
        ...
        for (let x = 0; x < partitions; x++) {
            ...
            L.rectangle([[lowerLat, lowerLong], [upperLat, upperLong]], {color: "orange", weight: 1}).addTo(map)
            .bindPopup(x + "," + y);
        }
    }
}
...
async function update() {
    ...
    fetch("/livemap/data")
        .catch(() => {
            console.log("Unable to update, please try again later");
        })
        .then(res => {
            if (res == undefined) {
                return;
